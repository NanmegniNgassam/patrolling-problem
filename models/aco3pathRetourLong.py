...
def aco_tsp(distance_matrix, start_node, graph, num_ants=100, num_iterations=100, alpha=1.0, beta=2.0, rho=0.5, Q=100):
    ...
    for _ in tqdm(range(num_iterations), desc="Optimizing"):
        ...
        for solution in solutions:
            ...
            # Idée : Pénaliser le chemin quand il repasse par des points
            # # Compter les doublons
            # counts = {}
            # for j in range(num_agents):  # Pour chaque agent
            #     for point in solution[j]:
            #         if point in counts:
            #             counts[point] += 1
            #         else:
            #             counts[point] = 1
            # # Ajouter 100 pour chaque point qui se répète
            # for point, count in counts.items():
            #     if count > 1:
            #         path_length += 100 * (count - 1)
...
def generate_path():
    ...
    for i in range(len(path)):
        ...
        reversed_path = path[i][::-1][1:]
        path[i] = path[i] + reversed_path
    print(f"Chemin trouvé : {path}")
    all_path.append(path)
