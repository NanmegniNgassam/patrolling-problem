...
# Renvoie laclé de la which-derniere plus petite valeur du dictionnaire
def find_key_of_first_none_or_penultimate_min_key(dict_values, which):
    # Vérifier d'abord pour `None`
    for key, value in dict_values.items():
        if value is None:
            return key  # Retourne immédiatement la clé si la valeur est None

    # Trier toutes les valeurs, en considérant None comme infini pour le tri
    sorted_items = sorted(dict_values.items(), key=lambda x:  x[1])

    # Vérifier si nous avons assez d'éléments pour obtenir un avant-dernier
    if len(sorted_items) < 2:
        return None  # Retourne None si il n'y a pas assez de valeurs pour avoir un avant-dernier
    else:
        # L'avant-dernière paire contient la clé de l'avant-dernière plus petite valeur
        penultimate_min_key = sorted_items[which][0]
        return penultimate_min_key
    ...
    while True:
        ...
        if distance < agent_speed:
            ...
            if not shared_list_chemins[agent_id]:
                ...
                with lock :
                    target_index = find_key_of_first_none_or_penultimate_min_key(last_visited_shared, 0)
                ...
                if target_index in shared_list_node_target:
                    ...
                    while not target_index:
                        target_index = find_key_of_first_none_or_penultimate_min_key(last_visited_shared, i)
                        if target_index in shared_list_node_target:
                            target_index = None
                        i+=1
            ...
            print(f"Je suis actuellement au noeud {agent_node_index}")
            print(f"Mon node target : {target_index}")
            print(f"Mon chemin pour y aller : {shared_list_chemins}")
